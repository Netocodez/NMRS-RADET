...
def UbuntuLineListToRatet():
        ...
        df.insert(0, 'S/N.', df.index + 1)
        #df.insert(4, 'Patient id', df.index + 1)
        df['Patient id']=df['uuid']
        df['Household Unique No']=''
        df['Received OVC Service?']=''
        df.loc[df['Sex']=="M",'Sex']='Male'
        df.loc[df['Sex']=="F",'Sex']='Female'
        df['IIT DATE'] = pd.to_datetime(df['Pharmacy_LastPickupdate']) + pd.to_timedelta(pd.to_numeric(df['DaysOfARVRefill']), unit='D') + pd.Timedelta(days=29)
        df['DaysOfARVRefill']=df['DaysOfARVRefill'].astype('float')
        df['DaysOfARVRefill'] = (df['DaysOfARVRefill'] / 30).round(1)
        df['TPT Type']=df['Current_TPT_Received']
        df['TPT Completion date (yyyy-mm-dd)']=''
        df['Date of Regimen Switch/ Substitution (yyyy-mm-dd)']=''
        df['Date of Full Disclosure (yyyy-mm-dd)']=''
        df['Number of Support Group (OTZ Club) meeting attended']=''
        ...
        df['Date calculated (yyyy-mm-dd)']=''
        df['Case Manager']=''
        df['Confirmed Date of Previous ART Status (yyyy-mm-dd)'] = ''
        df.loc[(df['CurrentARTStatus'] == 'LTFU') & (df['Outcomes_Date'].isna()), 'Outcomes_Date'] = df['IIT DATE']
        df.loc[(df['CurrentARTStatus'] == 'Active') & (df['Outcomes_Date'].isna()), 'Outcomes_Date'] = df['Pharmacy_LastPickupdate']
        df.loc[df['CurrentARTStatus']=="Death",'CurrentARTStatus']='Dead'
        df.loc[df['CurrentARTStatus']=="Discontinued Care",'CurrentARTStatus']='Stopped'
        df.loc[df['CurrentARTStatus']=="Transferred out",'CurrentARTStatus']='Transferred Out'
        df.loc[df['CurrentARTStatus']=="LTFU",'CurrentARTStatus']='IIT'
        df.loc[df['ARTStatus_PreviousQuarter']=="LTFU",'ARTStatus_PreviousQuarter']='IIT'
        df.loc[df['ARTStatus_PreviousQuarter']=="Discontinued Care",'ARTStatus_PreviousQuarter']='Stopped'
        df.loc[df['ARTStatus_PreviousQuarter']=="Transferred out",'ARTStatus_PreviousQuarter']='Transferred Out'
        df.loc[df['ARTStatus_PreviousQuarter']=="Death",'ARTStatus_PreviousQuarter']='Dead'
        ...
        df.loc[df['CurrentRegimenLine']=="Child 2nd line ARV regimen",'CurrentRegimenLine']='Peds.2nd.Line'
        df.loc[df['CurrentRegimenLine']=="Child 3rd line ARV regimen",'CurrentRegimenLine']='Peds.3rd.Line'
        df.loc[df['RegimenLineAtARTStart']=="Adult 1st line ARV regimen",'RegimenLineAtARTStart']='Adult.1st.Line'
        df.loc[df['RegimenLineAtARTStart']=="Adult 2nd line ARV regimen",'RegimenLineAtARTStart']='Adult.2nd.Line'
        df.loc[df['RegimenLineAtARTStart']=="Adult 3rd line ARV regimen",'RegimenLineAtARTStart']='Adult.3rd.Line'
        df.loc[df['RegimenLineAtARTStart']=="Child 1st line ARV regimen",'RegimenLineAtARTStart']='Peds.1st.Line'
        df.loc[df['RegimenLineAtARTStart']=="Child 2nd line ARV regimen",'RegimenLineAtARTStart']='Peds.2nd.Line'
        df.loc[df['RegimenLineAtARTStart']=="Child 3rd line ARV regimen",'RegimenLineAtARTStart']='Peds.3rd.Line'
        
        #VL CASCADE
        df['CurrentViralLoad(c/ml)'] = df['CurrentViralLoad']
        df['Date of Current Viral Load (yyyy-mm-dd)'] = df['DateResultReceivedFacility']
        ...
        #Add restarts, transfer-in and status at registration to current ART status in Radet
        df.loc[(df['ARTStatus_PreviousQuarter'].isin(["IIT", "Stopped", "Transferred out"])) & (df['CurrentARTStatus']== "Active"),'CurrentARTStatus']='Active-Restart'
        df.loc[df['CurrentARTStatus']=="Active-Restart",'Outcomes_Date']=df['Pharmacy_LastPickupdate']
        df.loc[(df['TransferredIn'] == 'Yes') & ((df['CurrentARTStatus']== "Active")),'CurrentARTStatus']='Active-Transfer In'
        df.loc[(df['Status at Registration']=='') & (df['TransferredIn'] == 'Yes'),'Status at Registration']='ART Transfer In'
        ...
        df = df[['S/N.',
                    ...
                    'DaysOfARVRefill',
                    'First_TPT_Pickupdate',
                    'Current_TPT_Received',
                    'TPT Completion date (yyyy-mm-dd)',
        ...
        #Convert Date Objects to Date
        dfDates = ["Date of Birth (yyyy-mm-dd)", "ART Start Date (yyyy-mm-dd)", "Last Pickup Date (yyyy-mm-dd)", "Date of TPT Start (yyyy-mm-dd)", "TPT Completion date (yyyy-mm-dd)", "Date of Regimen Switch/ Substitution (yyyy-mm-dd)", "Date of Full Disclosure (yyyy-mm-dd)", "Date Enrolled on OTZ (yyyy-mm-dd)", "Date of Viral Load Sample Collection (yyyy-mm-dd)", "Date of Current Viral Load (yyyy-mm-dd)", "Date of VL Result After VL Sample Collection (yyyy-mm-dd)", "Date of Enrollment/Transfer-In (yyyy-mm-dd)", "Confirmed Date of Previous ART Status (yyyy-mm-dd)", "Date of Current ART Status (yyyy-mm-dd)", "Date Commenced DMOC (yyyy-mm-dd)", "Date of Return of DMOC Client to Facility (yyyy-mm-dd)", "Date of Commencement of EAC (yyyy-mm-dd)", "Date of 3rd EAC Completion (yyyy-mm-dd)", "Date of Extended EAC Completion (yyyy-mm-dd)", "Date of Repeat Viral Load - Post EAC VL Sample Collected (yyyy-mm-dd)", "Date of Cervical Cancer Screening (yyyy-mm-dd)", "Date of Precancerous Lesions Treatment (yyyy-mm-dd)", "Date Biometrics Enrolled (yyyy-mm-dd)", "Date calculated (yyyy-mm-dd)"]
        for col in dfDates:
            df[col] = pd.to_datetime(df[col],errors='coerce').dt.date
...
def UbuntuLineListToRatet_Baseline():
        start_time = time.time()  # Start time measurement
        startDate, endDate = get_selected_date()
        status_label.config(text=f"Attempting to read file...")
        input_Bseline = filedialog.askopenfilename(initialdir = '/Desktop', 
        ...
        #Remove leading zeros from Hospital Numbers and Unique IDs that are digits 
        df['PatientHospitalNo1'] = df['PatientHospitalNo'].apply(lambda x: str(x).lstrip('0') if str(x).isdigit() else x)
        df['PatientUniqueID1'] = df['PEPID'].apply(lambda x: str(x).lstrip('0') if str(x).isdigit() else x)
        dfbaseline['Hospital Number1'] = dfbaseline['Hospital Number'].apply(lambda x: str(x).lstrip('0') if str(x).isdigit() else x)
        dfbaseline['Unique ID1'] = dfbaseline['Unique ID'].apply(lambda x: str(x).lstrip('0') if str(x).isdigit() else x)
        ...
        df.loc[((df['Date of Current Viral Load (yyyy-mm-dd)'].notnull()) & (df['CurrentViralLoad(c/ml)'] <= 1000)),'ViralLoadIndication']='Routine - Routine'
        df.loc[((df['Date of Current Viral Load (yyyy-mm-dd)'].notnull()) & (df['CurrentViralLoad(c/ml)'] > 1000)),'ViralLoadIndication']='Targeted - Post EAC'
        df.loc[((df['First_TPT_Pickupdate'].isna()) & (df["Date of TPT Start (yyyy-mm-dd)_baseline"].notnull())) | (df["Date of TPT Start (yyyy-mm-dd)_baseline"] > df['First_TPT_Pickupdate']), 'First_TPT_Pickupdate'] = df["Date of TPT Start (yyyy-mm-dd)_baseline"]
        #df.to_excel("test.xlsx")
        #dfbaseline.to_excel("test2.xlsx")
...
"NMRS-ART-LINE LIST TO RADET (ONLY CONVERTS TO RADET) ▼", 
"NMRS-ART-LINE LIST TO RADET (APPENDS BASELINE INFO (SELECT RADET BEFORE NMRS LINE LIST)) ▼", 
"CLEAN NMRS-ART-LINE LIST DATA TYPES (FROM .csv, .xls, .xlsx TO EXCEL(.xlsx)) ▼",
"Ubuntu NMRS-ART-LINE LIST TO RADET (FROM .csv, .xls, .xlsx TO EXCEL(.xlsx)) ▼",
"Ubuntu NMRS-ART-LINE LIST TO RADET+ (FROM .csv, .xls, .xlsx TO EXCEL(.xlsx)) ▼",
"CLEAN Ubuntu NMRS-ART-LINE LIST DATA TYPES (FROM .csv, .xls, .xlsx TO EXCEL(.xlsx)) ▼")
...
def on_dropdown_click():
    ...
    if selected_value == "NMRS-ART-LINE LIST TO RADET (ONLY CONVERTS TO RADET) ▼":
        ...
        CleanARTLineList()
    elif selected_value == "Ubuntu NMRS-ART-LINE LIST TO RADET (FROM .csv, .xls, .xlsx TO EXCEL(.xlsx)) ▼":    
        UbuntuLineListToRatet()
    elif selected_value == "Ubuntu NMRS-ART-LINE LIST TO RADET+ (FROM .csv, .xls, .xlsx TO EXCEL(.xlsx)) ▼":    
        UbuntuLineListToRatet_Baseline()        
